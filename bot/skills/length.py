...
class DB:
    ...
    def get_best(self, n=10):
        return list(self._coll.find({}).limit(n).sort("peninsula_length", pymongo.ASCENDING))
    ...
    def add(self, user: User):
        return self._coll.update_one(
            ...
            {"$set": {
                "meta": user.to_dict(),
                "peninsula_length": len(str(user.id))
            }},
            upsert=True,
        )
...
def _length(update: Update, context: CallbackContext):
    ...
    if update.effective_message is not None:
        result = update.effective_message.reply_text(f'Длина айди твоего телеграмм акаунта {len(str(user.id))} 🍆 ({str(user.id)})')
...
def _longest(update: Update, context: CallbackContext):
    ...
    for col in _db.get_best(10):
        ...
        peninsula_formatted = '\[' + ('🍆' * (longest_peninsula - int(col['peninsula_length']))) + '\]'
