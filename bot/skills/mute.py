...
def add_mute(application: Application, handlers_group: int):
    ...
    application.add_handler(ChatCommandHandler("mute", mute_self), handlers_group)
    application.add_handler(
        ChatCommandHandler(
            "unmute",
            unmute,
...
async def mute_self(update: Update, context: CallbackContext):
    ...
    result = await update.message.reply_text(choice(self_mute_messages))
