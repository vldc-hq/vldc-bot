...
async def _handle_false_start(
    ...
    if false_starts == 1:
        text = (
            f"😼 Придержи лапу, {user.full_name}. "
            "Настоящие гусары стреляют по очереди. "
            "Ещё один тык — и поедешь в переноске."
        )
    else:
        text = (
            f"😾 Дотыкался, {user.full_name}. "
            "Пять минут в переноске, без почестей и гусарских дней."
        )
